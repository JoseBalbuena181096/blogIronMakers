...
export default function UserProgress({ userId }: { userId: string }) {
    ...
    const [selectedCursoFilter, setSelectedCursoFilter] = useState<string>('');
    ...
    const cargarDatos = async () => {
        ...
        // Cargar intentos de quiz con información del curso
    ...
    // Get unique courses from quiz attempts
    const cursosConQuizes = Array.from(
        new Set(intentosQuiz.map(q => q.entradas?.curso_id).filter(Boolean))
    ).map(cursoId => {
        const quiz = intentosQuiz.find(q => q.entradas?.curso_id === cursoId);
        return {
            id: cursoId,
            titulo: quiz?.entradas?.cursos?.titulo || 'Sin título'
        };
    });

    // Filter quizzes by selected course
    const intentosFiltrados = selectedCursoFilter
