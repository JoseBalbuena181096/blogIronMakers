...
serve(async (req) => {
    ...
    try {
        ...
        // 3. Obtener entrada_id
        const { entrada_id } = await req.json()

        if (!entrada_id) {
            return new Response(
                JSON.stringify({ error: 'entrada_id is required' }),
                { status: 400, headers: { 'Content-Type': 'application/json' } }
            )
        }

        // 4. Forward al backend de Railway
        const response = await fetch(`${RAILWAY_BACKEND_URL}/api/v1/ingest`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify({ entrada_id }),
        })
